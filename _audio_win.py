...
def current_output():
    ...
    try:
        with warnings.catch_warnings():
            ...
            # pycaw >= 20240316 returns an AudioDevice wrapper carrying .id; older versions
            # handed back the raw IMMDevice, whose id comes from GetId(). Without the first
            # branch this silently returned nothing, and "next sound device" then always
            # stepped from the start of the list instead of from the device in use.
            device_id = speakers.id if hasattr(speakers, "id") else speakers.GetId()
